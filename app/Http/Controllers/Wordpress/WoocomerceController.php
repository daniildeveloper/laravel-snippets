...
class WoocomerceController extends Controller
{
    ...
    {
        $client = new Guzzle();
        \Log::info('Beginn parsing amirsana.kz');
        ini_set('allow_url_fopen', '1');
        $shop = $this->remoteShop;
        include_once "simplehtmldom/simple_html_dom.php";
        $base_url_single = "http://amirsana.kz";
        // dd($base_url_single);
        $html = $client->get($base_url_single)->getBody();
        $html = str_get_html($html);
        // dd($html);
        //берем все менюшки
        $menu = $html->find(".desktop-catalog-menu .menu-item a");
        \Log::info('get all data from main page');
        ...
        foreach ($menu as $menuItem) {
            ...
            $sizFirstChildCategoryShopIndex = $client->get($base_url_single . $sizFirstChildCategoryLinkToShop)->getBody();
            $sizFirstChildCategoryShopIndex = str_get_html($sizFirstChildCategoryShopIndex);
            $secondChildCategoriesList      = $sizFirstChildCategoryShopIndex->find("ul.catalog-menu li.expanded ul.children-items li a");
            \Log::info('New menu item');
